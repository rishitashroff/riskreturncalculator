...
```{r}
filtered_sp500 <- read.csv('filtered_sp500_stocks.csv')
colnames(filtered_sp500)[1] <- 'Ticker'


```

```{r}
# Find related stocks in sector of a company
related_stocks <- function(stock) {
sector <- filtered_sp500$Sector[filtered_sp500$Ticker == stock]

price <- filtered_sp500$Last_Price[filtered_sp500$Ticker == stock]
range = .25
...
related <- filtered_sp500$Ticker[filtered_sp500$Sector == sector]
related <- sector_specific_stocks[sector_specific_stocks != stock]

related_sector <- filtered_sp500[filtered_sp500$Ticker %in% related &
                                 filtered_sp500$Last_Price >= price_lower_bound &
                                 filtered_sp500$Last_Price <= price_upper_bound, ]

return(related_sector$Ticker)
}

s <- related_stocks('AAPL')
s
...
stdev <- na.omit(stdev)
colnames(stdev) <- trimws(colnames(stdev))

```


```{r}
stdev_filtered <- stdev[, colnames(stdev) %in% s]
stdev_filtered
variance_filtered <- stdev_filtered^2
variance <- unlist(variance_filtered)
...
alpha_prior <- 1
beta_prior <- mean(flatten_variance) # .0003
